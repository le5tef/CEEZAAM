...
async function fetchSelected() {
  if (typeof route.params.id === 'string') await clientsStore.setSelectedClient(route.params.id)
}
...
watch(() => route.params.id, fetchSelected)
onMounted(() => {
  fetchSelected()
})
...
async function save() {
  ...
  if (form_valid) {
    await clientsStore.saveTempClient()
  }
}
</script>
...
<template>
  <div>
    <div :class="['heading', 'font-26', 'd-flex', 'align-center']">
      ...
      <div>Редактирование клиента</div>
    </div>
    ...
    <ActionButtons @submit="save" @reset="clientsStore.resetTempUser" />
  </div>
</template>
